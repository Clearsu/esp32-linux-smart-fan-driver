...
void app_main(void)
{
	...
	ret = xTaskCreate(sensor_task, "sensor_task", 4096, NULL, 3, NULL);
	if (ret != pdPASS)
		ESP_LOGE("MAIN", "Failed to create sensor_task");
	ret = xTaskCreate(fan_control_task, "fan_control_task", 4096, NULL, 4, NULL);
	if (ret != pdPASS)
	...
	ret = xTaskCreate(uart_read_task, "uart_read_task", 4096, NULL, 6, NULL);
	if (ret != pdPASS)
