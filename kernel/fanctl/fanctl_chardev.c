...
static inline u16	fanctl_be16(const u8 *p)
{
...
static long fanctl_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FANCTL_IOC_PING:
		return fanctl_do_req_wait_resp(ctx, PROTO_CMD_PING,
				NULL, 0, &resp, msecs_to_jiffies(1000));
	case FANCTL_IOC_GET_STATUS: {
		...
		if (resp.cmd != PROTO_CMD_STATUS_RESP || resp.len < sizeof(status_resp_t))
			return -EPROTO;
		p = resp.payload;
		st.temp_x100 = (u16)fanctl_be16(p + 0);
		st.humidity_x100 = (u16)fanctl_be16(p + 2);
		st.fan_mode = p[4];
		st.fan_state = p[5];
		st.errors = (u16)fanctl_be16(p + 6);
	...
	case FANCTL_IOC_SET_FAN_MODE: {
		...
		ret = fanctl_do_req_wait_resp(ctx, PROTO_CMD_SET_FAN_MODE,
				payload, 1, &resp, msecs_to_jiffies(1000));
		if (ret) return ret;

		if (resp.cmd != PROTO_CMD_ACK || resp.len < 2)
			return -EPROTO;
		return (resp.payload[1] == PROTO_ERR_OK) ? 0 : -EPROTO;
	}
	...
	case FANCTL_IOC_SET_THRESHOLD: {
		...
		ret = fanctl_do_req_wait_resp(ctx, PROTO_CMD_SET_THRESHOLD,
				payload, 2, &resp, msecs_to_jiffies(1000));
		if (ret)
			return ret;
		if (resp.cmd != PROTO_CMD_ACK || resp.len < 2)
			return -EPROTO;
...
#ifdef CONFIG_COMPAT
	.compat_ioctl = fanctl_unlocked_ioctl,
#endif
};
